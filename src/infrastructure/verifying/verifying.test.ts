import { KeyPair } from "../../record/entity/document/signature"
...
describe('Verifying jws tests', () => {
  ...
  const keyPair: KeyPair = { privateKey: privateKey, publicKey: publicKey }
  ...
  it('JWSVerify_valid_without_explicit_public_key', async () => {
    const keyPairNoPublic: KeyPair = { privateKey: privateKey, publicKey: '' }
    const signing: SigningClient = new Signing()
    let response = await signing.JWSSign(keyPair, payload, headers)
  ...
  it('JWSVerify_invalid_key_pairs_matching', async () => {
    ...
    const invalidkeyPair: KeyPair = { privateKey: privateKey, publicKey: invalidPublicKey }
    const payload: string = '32e3971ea882d3e46ed788888cfa747958a0999c0338c98126a5ed57f53bee60'
